...
- Fixed AVX2 wave size 8 f16->f32 conversions.
- Fixed geometry shaders running at wave sizes 4 or 8 on x86/x64.

### Version 1.0.16

#### Codegen improvements

When compiling for x86/x64, WARP will now optionally use larger registers to store 32-bit data, which effectively increases its native wave size. Minimum wave size is still 4,
but maximum is now determined by CPU capability: AVX+AVX2 supports wave size 8, and AVX512 supports wave size 16. WARP will choose the largest wave size that makes sense:

- For vertex pipeline shaders (VS/GS/HS/DS) and raytracing, the maximum size is used.
- For pixel shaders, the maximum size is used unless the output render target is tiled. A single wave will only execute quads from a single triangle in a single horizontal row.
- For compute and compute-like shaders (MS/AS), the wave size can be explicitly selected from DXIL annotations. Barring those, the size is the largest that is not greater than
the thread group size (no use having 16 threads per wave in a group with only 4 threads!).

WARP also now uses better instruction sequences for a wide array of wave ops across SSE, AVX, AVX512, and arm64, including native wave op accumulations for arm64.

#### Bug fixes

- Fix D3D12 `CalculateLevelOfDetailUnclamped()` return value
- Fix computing variables in control flow in multi-iteration shaders (pixel shaders at sample frequency, or geometry shaders with multiple iterations)

### Version 1.0.15.1

- Fix `WaveReadLaneAt()` and similar methods for fp16

### Version 1.0.15

- Correctly handle `switch()` on 64-bit values
- Correctly handle raytracing BLAS creation with an empty geometry input
- Fix a crash while compiling compute shaders with non-uniform control flow
- Another FMA fix for some cases when two sources are the same
- Implement `mad()` with double-precision floating point
- Prevent a crash with mesh shader pipelines that do not have a pixel shader, but still need rasterization (e.g. depth-only rasterization)
- Fix `firstbithi()` result when input is `-1ull`
- Fix `IncrementCounter()` and `DecrementCounter()` when the object is a divergent resource access between threads in a wave

### Version 1.0.14.2

- A couple more FMA bugfixes
- Additional raytracing codegen improvements to keep more data in registers
