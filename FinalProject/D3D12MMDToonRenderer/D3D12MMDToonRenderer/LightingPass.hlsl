    ...
    float isCharacter)
{
    ...
    const float3 biasedWorldPos =
        worldPos +
        N * normalOffset;

    float4 lightSpacePos =
        mul(
            float4(biasedWorldPos, 1.0f),
            lightViewProj);
    ...
    for (int y = -1; y <= 1; ++y)
    {
        ...
        {
            visibility +=
                g_shadowMap.SampleCmpLevelZero(
                    g_shadowSampler,
                    shadowUV +
                        float2(
                            float(x),
...
float4 PSLighting(PSInputLight input) : SV_TARGET
{
    ...
    float3 albedo =
        g_gbufferAlbedo.Sample(
            g_sampler,
            input.uv).rgb;

    float4 normalSample =
        g_gbufferNormal.Sample(
            g_sampler,
            input.uv);
    ...
    if (isCharacter > 0.5f)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            float4 neighborNormalSample =
                g_gbufferNormal.SampleLevel(
                    g_sampler,
                    neighborUV,
                    0);
            ...
            float3 neighborNormal =
                normalize(
                    neighborNormalSample.xyz *
                    2.0f -
                    1.0f);

            float3 neighborPosition =
                g_gbufferPos.SampleLevel(
                    g_sampler,
                    neighborUV,
                    0).xyz;
        ...
        float characterAmbientVisibility =
            lerp(
                0.45f,
                1.0f,
                saturate(ao));
        ...
        finalColor =
            albedo *
            0.34f *
            characterAmbientVisibility;

        if (lightsEnabled > 0.5f)
        {
            const float halfLambert =
                saturate(
                    dot(normal, L) *
                    0.5f +
                    0.5f);
            ...
            const float toonRamp =
                smoothstep(
                    0.36f,
                    0.58f,
                    halfLambert);

            const float toonBand =
                lerp(
                    0.18f,
                    0.66f,
                    toonRamp);
            ...
            const float filteredCharacterShadow =
                smoothstep(
                    0.10f,
                    0.90f,
                    shadowFactor);

            const float characterShadowVisibility =
                lerp(
                    1.0f,
                    filteredCharacterShadow,
                    CHARACTER_SHADOW_MAP_STRENGTH);

            const float toonDirect =
                toonBand *
                characterShadowVisibility;
